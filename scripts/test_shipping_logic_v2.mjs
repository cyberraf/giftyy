        ...
        for (const zone of zones) {
            ...
            if (countries.length === 0 || countries.includes(normalizedCountry)) {
                if (normalizedCountry === 'United States' && Array.isArray(zone.us_states) && zone.us_states.length > 0) {
                    const includesState = zone.us_states.includes(normalizedState);
                    console.log(`Checking zone ${zone.id}: state ${normalizedState} in ${JSON.stringify(zone.us_states)}? ${includesState}`);
                    if (!includesState) {
                        continue;
                    }
                }
...
async function runTest() {
    ...
    fs.writeFileSync('scripts/test_shipping_result.json', JSON.stringify({ res1, res2 }, null, 2));
    console.log('Done');
}
