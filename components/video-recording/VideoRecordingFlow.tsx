/**
 * Giftyy Video Recording Flow
 * Premium, emotional, gift-centered video recording experience
 * Record-only (no upload option), 30-second maximum duration
 */
...
export function VideoRecordingFlow({ onVideoRecorded, onCancel, initialVideoUri, initialDurationMs, onRetake }: VideoRecordingFlowProps) {
	...
	// Start recording (moved before countdown to avoid dependency issue)
		...
		try {
			await cameraRef.current?.startRecording({
				...
				onRecordingFinished: (video) => {
					// @ts-ignore - different platforms return different structures
	...
	// Start recording (public wrapper)
	const startRecording = useCallback(async () => {
		if (recording) return;
		await startRecordingInternal();
	}, [recording, startRecordingInternal]);
	...
	// Handle record button press - always start with countdown
	const handleRecordPress = useCallback(() => {
		if (recording) {
			stopRecording();
		} else {
			// Always start with countdown
			startCountdown();
		}
	...
	const handleUseVideo = useCallback(async () => {
		if (videoUri) {
			// Stop video playback before proceeding
			try {
				if (previewRef.current) {
					await previewRef.current.pauseAsync();
					await previewRef.current.unloadAsync();
	...
	const getTimerColor = (progress: number) => {
		if (progress > 0.83) return '#ef4444'; // 25-30s: red
		if (progress > 0.67) return '#fbbf24'; // 20-25s: yellow
		return PRIMARY; // 0-20s: calm
	};
	...
	if (screen === 'welcome') {
		return (
			<View style={styles.container}>
				<LinearGradient
					colors={[GIFTYY_THEME.colors.cream, GIFTYY_THEME.colors.softPink]}
		...
		<View style={styles.container} collapsable={false}>
			...
			{recording && (
				<View style={styles.timerContainer}>
					<Text style={[styles.timerText, { color: getTimerColor(progress) }]}>
						{Math.floor(elapsedMs / 1000)}s
					</Text>
					{remainingSeconds <= 3 && remainingSeconds > 0 && (
						<Text style={[styles.countdownWarning, { color: '#ef4444' }]}>
							{remainingSeconds}
						</Text>
					)}
				</View>
